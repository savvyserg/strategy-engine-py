...
class CSVCandleSourceAdapter(CandleSourcePort):
    ...
    def __init__(self, file_name: str = 'input.csv'):
        ...
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"{type(self).__name__} Error: CSV candle source file not found at: '{file_path}'.")
        ...
        try:
            with open(self._file_path, mode='r', newline='', encoding='utf-8') as f:
                ...
                if not reader.fieldnames or not required.issubset(set(reader.fieldnames)):
                    ...
                    raise ValueError(f"{type(self).__name__} Error: CSV candle source file at '${self._file_path}' missing required columns '{missing}'.")
            ...
            raise FileNotFoundError(f"{type(self).__name__} Error: CSV candle source file not found at '{self._file_path}'.")
